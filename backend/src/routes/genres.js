...
router.get('/:genreId', async (req, res) => {
  try {
    ...
    const [movies, series, totalMovies, totalSeries] = await Promise.all([
      Movie.find({ genres: genreId, voteCount: { $gt: 100 }  })
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 }),
      Series.find({ genres: genreId, voteCount: { $gt: 100 }  })
        .skip(skip)
        .limit(limit)
        .sort({ createdAt: -1 }),
    ...
    res.status(500).json({ error: 'Failed to fetch genre details', details: error.message });
  }
});
